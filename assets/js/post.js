...
// Link verification for Stories
    $form.find(":input[name='link']").on("keyup", function() {
    	var link = $form.find(":input[name='link']").val().toLowerCase();
    	var link_check = /^(http|https):\/\/[a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.[a-z]{2,20}(:[0-9]{1,5})?(\/.*)?$/i;
    	var link_empty = "";
    	
    	if (link !== link_empty) {
    		if ((link.substr(0,7) !== "http://") && (link.substr(0,8) !== "https://")) {
            	$form.find(":input[name='link']").val("http://"+link);
 ...
 if (type == "story") {
            ...
            $form.find(":input[name='link']").prop("disabled", false);
            $form.find(".js-link-worked").css({"display": "inherit"});
	    $form.find(".js-link-not-worked").css({"display": "inherit"});
        } else {
            $form.find(".tabs .emojionearea").removeClass('disabled');
            $form.find(":input[name='location-search']").prop("disabled", false);
            $form.find(":input[name='link']").prop("disabled", true);
            $form.find(".js-link-worked").css({"display": "none"});
	    $form.find(".js-link-not-worked").css({"display": "none"});
        }
    });
	...
	data.link = $form.find(":input[name='link']").val().toLowerCase();
