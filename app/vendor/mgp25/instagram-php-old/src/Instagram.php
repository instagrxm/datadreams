...
/**
 * Nextpost Private API for Instagram v.1.0.5 (customized)
 * 
 * @version NEXTPOST
 * Last update: 7.02.2020
 *
 * Each buyer of Nextpost PHP-script (originally developed by Postcode) 
 * have a full MIT license for the script and valid premium licenses 
 * for used libraries without it's expiration date.
 *
 */
class Instagram implements ExperimentsInterface
{
        ...
        Response\LoginResponse $response)
    {
        ...
        if (null !== $response->getLoggedInUser()) {
            ...
            throw new \InvalidArgumentException('getLoggedInUser() parameter in login response is empty.');
        }
        ...
        $appRefreshInterval = 1800)
    {
        ...
        if ($justLoggedIn) {
            ...
            // $this->story->getReelsMediaFeed($this->account_id); this request already done before
    ...
    /**
     * Send custom event to Facebook Graph API 
     * 
     * This file should be placed directly in Instagram.php or we shoud add some missing parameters.
     * 
     * @param string    $event_name     'fb_mobile_activate_app' - open Instagram app
     *                                  'reel_playback_entry' - open Instagram Stories reel
     *
     * @return mixed
     */

    public function fbEventRequest(
        $event_name = null)
    {
        if (empty($event_name)) {
            throw new \InvalidArgumentException('You must provide a valid event name to fbEventRequest().');
        }

        $endpoint = Constants::FACEBOOK_ORCA_APPLICATION_ID.'/activities/';

        $data = [
            '_appVersion'  =>  Constants::IG_VERSION,
            '_logTime'     =>  time(), // timestamp
        ...
        return $this->request($endpoint)
                    ...
                    ->addFileData('custom_events_file', json_encode($data), 'custom_events_file')
                    ->addPost('format', 'json')
                    ->addPost('advertiser_tracking_enabled', 1)
                    ->addPost('anon_id', 'XZ'.$this->fb_tracking_id)
                    ->addPost('advertiser_id', $this->advertising_id)
                    ->addPost('event', 'CUSTOM_APP_EVENTS')
                    ->addPost('application_package_name', Constants::PACKAGE_NAME)
                    ->addPost('application_tracking_enabled', 1)
                    ->getDecodedResponse(false);
    }
}
