...
class Location extends RequestCollection
{
        ...
        $maxId = null)
    {
		...
		// convert new response to old one: first the ranked results (standard in new API)
        
		$oldfeed = new Response\LocationFeedResponse();
		$mediaCounter = 0;
		
		$newfeed = $locationFeed->getResponse(new Response\NewLocationFeedResponse());
		
		$collectRankedItems = array();
		foreach ($newfeed->getSections() as $items) {
			foreach($items->getLayoutContent()->getMedias() as $item){
				array_push($collectRankedItems, $item->getMedia());
				$mediaCounter += 1;
			}
		}
		$oldfeed->setRankedItems($collectRankedItems);
		$oldfeed->setMediaCount($mediaCounter);
		echo 'Ranked Items:'.$mediaCounter."<br>";

// BEGIN commenting out here for speed -- and add above:		$oldfeed->setItems($collectRankedItems);
		// Call IG again for recent items 
		...
		$oldfeed->setItems($collectRecentItems);
		$locationStory = $this->ig->request("locations/{$locationId}/story/")->getResponse(new Response\LocationStoryResponse());
		$oldfeed->setStory($locationStory->getStory());		
		echo 'Story Feed Media:'.($locationStory->getStory()->getMediaCount())."<br>";
// END commenting out here for speed

		// set total count, other data and exit
		$oldfeed->setNumResults($mediaCounter);
		$oldfeed->setNextMaxId($newfeed->getNextMaxId());
		$oldfeed->setMoreAvailable($newfeed->getMoreAvailable());	
		
		echo 'Total Items:'.$mediaCounter."<br>";
