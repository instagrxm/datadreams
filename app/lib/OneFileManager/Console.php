...
class Console
{
    ...
    private function processVideo($filename)
    {
        ...
        if ($height < 315) {
            @unlink($file);
            Common::error(__("Video should be at least 315px height.")); 
        }

        // Check aspect ratio
        $feed = null;
        $ratio_is_valid = false;
        $hor_video_feed = false;
        $hor_video_story = false;
        ...
        {   
        	...
        	$ratio_is_valid = true;
        	$hor_video_feed = true;
        } else if ($ratio < \InstagramAPI\Media\Constraints\StoryConstraints::MIN_RATIO) {
        	$feed = "story";
            $ratio_is_valid = true;
            $hor_video_story = true;
        }
        ...
        if (get_option("np_video_processing")) {
            ...
            $save = false;
            if ($hor_video_feed) {
        		// Vertical video should be cropped for posting in Timeline feed
        		$ratio = \InstagramAPI\Media\Constraints\TimelineConstraints::MIN_RATIO;
        		...
        		$point_y = ($height - $new_height) / 2;
        		
        		$video->filters()->crop(
        			new \FFMpeg\Coordinate\Point(0, $point_y), 
            		new \FFMpeg\Coordinate\Dimension($width, $new_height)
            	);
        	...
        	if ($hor_video_story) {
        		// Vertical video should be cropped for posting in Story feed
        		$ratio = \InstagramAPI\Media\Constraints\StoryConstraints::MIN_RATIO;
        		$new_height = (int)($width / $ratio);
            ...
            if ($duration >= 60 && $feed !== "story") {
                // Video is too long for feed, should be clipped
                $video->filters()->clip(
			...
			if ($duration > 15 && $feed == "story") {
                // Video is too long for story, should be clipped
                $video->filters()->clip(
                    \FFMpeg\Coordinate\TimeCode::fromSeconds(0), 
            ...
            if ($ffprobe->streams($file)->videos()->first()->get("codec_name") != "h264" || $ext !== "mp4") {
                // Vide is not MP4 and codec is not h.264 => should be converted
                $save = true;
            }
