    ...
    public static function publish($Post)
    {
        ...
        if ($type == "album" && count($media_ids) < 2) {
            ...
            $Post->set("data", $msg)
                ->update();
            throw new Exception($msg);
        } elseif ($type == "story" && count($media_ids) < 1) {
            $msg = __("Couldn't find selected media for the story");
            $Post->set("data", $msg)
                ->update();
            throw new Exception($msg);
        } elseif ($type == "timeline" && count($media_ids) < 1) {
        ...
        try {
            if ($type == "timeline") {
                ...
                if (in_array($ext, ["mp4"])) {
                    ...
                    $listTag = $Post->get('tags');
                    $listTag = unserialize($listTag);
		...
		if ($link) {
            ...
            } else if ($type == "album") {                $album_media = [];
                $temp_files_handlers = [];
