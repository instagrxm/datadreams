...
class PostController extends Controller
{
    ...
    public function process()
    {
        ...
        $tagPeopleId = "tag-people";
        $userModules = $AuthUser->get("settings.modules");

        if(empty($userModules)) {
            $userModules = [];
        }

        $isTagPeopleActive = true;

        if(!in_array($tagPeopleId, $userModules)) {
            $isTagPeopleActive = false;
        }
        ...
        $this->setVariable("isTagPeopleActive", $isTagPeopleActive)
        ...
        if (Input::get("action") == "suggestion") {
            $this->suggestion();
        }
    ...
    private function transformSuggestion($item) {
        $user = $item->user;
        return [
            'pk' => $user->pk,
    ...
    /**
     * Show list of default suggestion
     */
    private function defaultSuggestion() {
        ...
        if (!$Account) {
            $this->resp->msg = __("Please select an Instagram account to get suggestion.");
            $this->jsonecho();
        }
        ...
        $users = array_slice($following->getUsers(), 0, 7);
        ...
        $this->resp->suggestions = $suggestions;
        $this->jsonecho();
    }

    /**
     * Search suggestion with query
     * @param $query
     */
    private function searchSuggestion($query) {
        $url = 'https://www.instagram.com/web/search/topsearch/?context=blended&query='.$query.'&rank_token=0.7025128643422078&include_reel=false';

        $response = $this->request($url);
        $responseJson = json_decode($response);
        $users = $responseJson->users;
        $suggestions = array_map(array($this, 'transformSuggestion'), $users);
